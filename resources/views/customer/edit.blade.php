...
@section('content')
    <section class="bg-white">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-xl font-bold text-gray-900">Update customer data</h2>
            @if (session('status'))
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="w-full">
                        ...
                        <input type="date" name="date_of_birth" id="date-of-birth"
                            ...
                            value="{{ old('date_of_birth') ?? $customer->tanggal_lahir }}">
                        @error('date_of_birth')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select name="sex" id="sex"
                            ...
                            <option value="L"
                                {{ (old('sex') ?? $customer->jenis_kelamin) == 'L' ? 'selected' : '' }}>Male</option>
                            <option value="P"
                                {{ (old('sex') ?? $customer->jenis_kelamin) == 'P' ? 'selected' : '' }}>Female</option>
                        </select>
                        @error('sex')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
